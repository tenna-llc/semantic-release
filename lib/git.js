...
/** Publish only the refs owned by one release. A failed atomic push changes none of them. */
async function pushAtomic(repositoryUrl, branch, tagName, expectedHead, execaOptions) {
  const branchRef = `refs/heads/${branch}`;
  const tagRef = tagName && `refs/tags/${tagName}`;
  const branchRemote = (await execa('git', ['ls-remote', '--refs', repositoryUrl, branchRef], execaOptions)).stdout;
  const branchLine = branchRemote && branchRemote.split('\n').find((line) => line.endsWith(`\t${branchRef}`));
  if (!branchLine || branchLine.split('\t')[0] !== expectedHead) {
    throw new Error(`Remote branch moved before atomic release: ${branchRef}`);
  }

  if (tagRef && (await execa('git', ['ls-remote', '--refs', repositoryUrl, tagRef], execaOptions)).stdout) {
    throw new Error(`Release tag already exists: ${tagRef}`);
  }

  const refs = [
    `HEAD:${branchRef}`,
    ...(tagRef ? [`${tagRef}:${tagRef}`] : []),
    `refs/notes/${GIT_NOTE_REF}:refs/notes/${GIT_NOTE_REF}`,
  ];
  await execa('git', ['push', '--atomic', repositoryUrl, ...refs], execaOptions);
  const expected = new Map();
  expected.set(branchRef, (await execa('git', ['rev-parse', 'HEAD'], execaOptions)).stdout);
  if (tagRef) expected.set(tagRef, (await execa('git', ['rev-parse', tagRef], execaOptions)).stdout);
  const notesRef = `refs/notes/${GIT_NOTE_REF}`;
  expected.set(notesRef, (await execa('git', ['rev-parse', notesRef], execaOptions)).stdout);
  const verified = (await execa('git', ['ls-remote', '--refs', repositoryUrl, ...expected.keys()], execaOptions))
    .stdout;
  ...
  for (const [ref, sha] of expected) {
    if (actual.get(ref) !== sha) throw new Error(`Could not verify atomic release ref: ${ref}`);
  }
}
